...
class TranslationReducer extends CachingWriter {
  constructor(inputNode, options) {
    ...
    console.log(`>>>>>>>>>> this.options.modularizeTranslations = ${this.options.modularizeTranslations}`);

    // Mandate checks
    if (this.options.modularizeTranslations === true) {
      if (this.options.wrapTranslationsWithNamespace !== true) {
        this._log(`
          'options.wrapTranslationsWithNamespace' has to be set to 'true' when setting 'options.modularizeTranslations' to 'true', for it to work
        `);
        this.options.modularizeTranslations = false; // reset, since invalid without 'wrapTranslationsWithNamespace=true'
      }
    }
  ...
  mergeTranslations(listFiles) {
    ...
    return orderedTranslations.reduce((accum, filepath) => {
      ...
      if (this.options.modularizeTranslations === true) {
        const moduleKey = Object.keys(translation)[0];
        const moduleValues = Object.values(translation)[0]; // TODO: validate Object.values(translation) array or object cases

        // Aggregate values under module-name
        return extend(true, accum, {
          [moduleKey]: { [localeName]: moduleValues },
        });
      } else {
        // Aggregate values under locale name
        return extend(true, accum, {
  ...
  build() {
    ...
    if (this.options.modularizeTranslations === true) {
      /*
        Object structure :
        translations = {
          'module-common': { 'en': { 'title': 'Common', ... }, ... },
          'module-tickets': { 'en': { 'title': 'Tickets', ... }, ... }
        }
      */
      for (const [moduleKey, moduleValues] of Object.entries(translations)) {
        // 1. create folder with module-key as name
        const moduleFolderPath = path.join(filepath, moduleKey);
        mkdirp.sync(moduleFolderPath);

        const fallbacks = moduleValues[this.options.fallbackLocale];

        // 2. fill the folder with locale.json files in it
        for (const [locale, localeValues] of Object.entries(moduleValues)) {
          if (this.options.verbose) {
            this.validateMessages(localeValues, locale);
          }

          if (fallbacks && this.options.fallbackLocale !== locale) {
            moduleValues[locale] = extend(true, {}, fallbacks, moduleValues[locale]);
          }

          fs.writeFileSync(`${moduleFolderPath}/${this.generateFilename(locale)}`, stringify(moduleValues[locale]), {
            encoding: 'utf8',
          });
        }
      }
      ...
      for (const locale in translations) {
        ...
        fs.writeFileSync(`${filepath}/${this.generateFilename(locale)}`, stringify(translations[locale]), {
          encoding: 'utf8',
        });
      }
    }
  ...
  validateMessages(messages, locale) {
    ...
    forEachMessage(messages, (key, message) => {
      try {
        ...
        this._logLocale(locale, `"${key}" message cannot be parsed: ${error.message}`, { warning: true });
      }
    });
  }
}
