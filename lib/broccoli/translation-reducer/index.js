    ...
    // Mandate checks
		if (this.options.modularizeTranslations === true) {
			if (this.options.wrapTranslationsWithNamespace !== true) {
				this._log(`'options.wrapTranslationsWithNamespace' has to be set to 'true' when setting 'options.modularizeTranslations' to 'true', for it to work`);
      ...
      for (const [moduleKey, moduleValues] of Object.entries(translations)) {
        ...
        for (const [locale, localeValues] of Object.entries(moduleValues)) {
          ...
          if (fallbacks && this.options.fallbackLocale !== locale) {
            localeValues = extend(true, {}, fallbacks, localeValues);
          }

          fs.writeFileSync(`${moduleFolderPath}/${this.generateFilename(locale)}`, stringify(localeValues), {
            encoding: 'utf8',
          });
        }
      }
